...
import com.min.terminal.entity.ExecInfo;
import com.min.terminal.utils.DockerUtil;
 ...
 */
@Component
public class ContainerExecHandler extends TextWebSocketHandler {

    private Map<String, ExecInfo> map = new HashMap<>();

    @Override
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        //获得传参
        String ip=session.getAttributes().get(Constants.IP).toString();
        String containerId=session.getAttributes().get(Constants.CONTAINER_ID).toString();
        String width=session.getAttributes().get(Constants.WIDTH).toString();
        String height=session.getAttributes().get(Constants.HEIGHT).toString();
        //创建bash
    ...
    private void getExecMessage(WebSocketSession session, String ip, String containerId, Socket socket) throws IOException {
        ...
        outPutThread.start();
        map.put(containerId,new ExecInfo(ip,containerId,socket,outPutThread));
    }
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus closeStatus) throws Exception {
        String containerId = session.getAttributes().get(Constants.CONTAINER_ID).toString();
        ExecInfo execSession = map.get(containerId);
        if(execSession != null){
